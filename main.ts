...
namespace superKitI2C {
    ...
    // --- NOVO: BLOCO PARA LER NÚMERO COMPOSTO DO TECLADO ---

    /**
     * Lê múltiplas teclas numéricas do teclado até que uma tecla de ação seja pressionada.
     * Retorna o número formado ou 0 se nenhuma tecla numérica for pressionada.
     * @param pcfAddr endereço I2C do PCF8574
     * @param teclaFinal tecla que finaliza a entrada (ex: #, A, B, C, D)
     */
    //% block="[Teclado] Ler número composto até tecla %teclaFinal no endereço %pcfAddr"
    //% pcfAddr.defl=0x20
    //% teclaFinal.defl="#"
    export function lerNumeroComposto(pcfAddr: number, teclaFinal: string): number {
        let numeroStr = "";
        let tecla = "";

        while (true) {
            tecla = lerTeclado(pcfAddr);
            if (tecla == "") {
                basic.pause(50);
                continue;
            }
            ...
            if (tecla >= "0" && tecla <= "9") {
                ...
                // Ignora letras de operação ou outras teclas
                basic.pause(200);
            }
        }

        if (numeroStr.length == 0) return 0;
